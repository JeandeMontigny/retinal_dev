  ...
  bool writeIndividualRI = true;
    ...
    for (int num_cells = start_num_cells; num_cells <= max_num_cells;
      ...
      for (int iteration = 0; iteration < numberOfIteration; iteration++) {
        int maxStep = 200;
        int cubeDim = 1500; // 500 // streaching: 1500
        ...
        for (int i = 0; i < num_cells/num_c_type; i++) {
          ...
          do {
            ...
            if (bi_layer) {
              z = random->Uniform(20, 35); // 20, 35
            }
            else {
              z = random->Uniform(20, 20); // 20, 35
            }
            position = {x, y, z};
        ...
        //
        for (int i = 0; i < num_cells-(num_cells/num_c_type); i++) {
          std::array<double, 3> position;
          do {
            double x = random->Uniform(500, 1000);
            double y = random->Uniform(500, 1000);
